...
- Persistence goes through the tenant store: `src/store.ts` (`TenantStore`/`FileStore`), reached via `currentTenant()` from `src/context.ts`. `src/storage.ts` is now just filesystem primitives + types. `src/tenant-state.ts` holds the device-id accessors and the pinned-delivery-address-id accessors (`readSelectedAddressId` / `writeSelectedAddressId` / `clearSelectedAddress`) that `api.ts` and `session.ts` consume. `store.lock(key, fn)` is not reentrant (`"auth"` vs `"device"`).
- Delivery coordinates are never stored locally or defaulted. `api.ts:resolveDeliveryAddress` fetches the account's saved addresses (`fetchAddresses` → `GET auth.sixty60.co.za/customers/<userId>/addresses`) and returns the pinned one, else the highest `lastUsedOn`. `settings.json` holds only `{ addressId, savedAt }`.
