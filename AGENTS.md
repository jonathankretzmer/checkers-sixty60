...
- OTP verify keys `target.identifier` on the exact string the pending `reference` was stored under, which is not necessarily the E.164 form the request sent. The observed request response is just `{ reference, expiry }` — no identifier echoed. So `verifyOtp` retries against each SA number format (`+27…`, `27…`, `0…`) via `otpIdentifierCandidates`, stopping early if the response mentions the OTP itself (wrong/expired code, attempts) so it doesn't burn attempts on a format that was actually accepted. If a future response *does* echo an identifier, `pickOtpIdentifier` captures it, it's persisted in `auth.json` (`otpIdentifier`) and threaded through as the sole candidate. A total failure throws with every attempt's status + body. Number formatting helpers: `phoneDigits` (canonical `27XXXXXXXXX`), `normalizePhone` (E.164), `toNationalPhone` (`0…`).
